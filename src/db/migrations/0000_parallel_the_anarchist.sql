...
CREATE UNIQUE INDEX `actions_rule_id_unique` ON `actions` (`rule_id`);--> statement-breakpoint
CREATE TABLE `conditions_tree` (
	`id` integer PRIMARY KEY NOT NULL,
	`type` text NOT NULL,
	...
	`value2` text,
	FOREIGN KEY (`rule_id`) REFERENCES `rules`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`parent_group_id`) REFERENCES `conditions_tree`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
